...
> - `daily_incremental` 的含义是“工作流可以按小时运行，但邮件/通知每天最多发送一次”；运行频率和发送频率是两层概念，不要混淆
...
on:
  schedule:
    - cron: "33 * * * *"  # 每小时第 33 分钟运行一次
```

> 💡 **重要说明**：
> 这里控制的是“工作流多久运行一次”，不是“邮件多久发一次”。
> 例如使用 `daily_incremental` 预设时，GitHub Actions 可以仍然每小时运行一次，但 TrendRadar 会利用去重状态控制“每天最多发送一次增量邮件”。
...
| 我想要... | 复制这行代码 | 说明 |
|-----------|------------|------|
| **每小时一次** | `- cron: "33 * * * *"` | **当前默认配置**，第 33 分钟运行 |
