#!/usr/bin/env python3
"""
Qdrant 知识库 Token 节省效果测试脚本（通过 API）

测试场景：
1. 模拟用户查询关于"知识库系统架构"的问题
2. 对比两种方案：
   - 方案A：直接将所有文档发给 LLM（无知识库）
   - 方案B：使用知识库 API 搜索，只发送相关片段
3. 计算 token 节省比例
"""
...
def test_token_savings():
    ...
    print("=" * 80)
    print("Qdrant 知识库 Token 节省效果测试（通过 API）")
    print("=" * 80)
    print()
    ...
    try:
        ...
        print("   请确保服务已启动: ./start_kb_server.sh")
        sys.exit(1)
    ...
    print(f"平均每文档: {total_size // len(all_documents) if len(all_documents) > 0 else 0:,} 字符")
    print()
    ...
    print(f"API 响应时间: {(t1-t0)*1000:.2f}ms")
    ...
    report = {
        ...
        "scenario_b": {
            "description": "使用 Qdrant 知识库 API",
            "results_count": len(results),
            "api_response_time_ms": float((t1-t0)*1000),
    ...
    report_path = Path("/tmp/qdrant_token_savings_api_report.json")
